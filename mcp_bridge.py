...
_MCP_INSTRUCTIONS = (
    ...
    "Model/effort switching (Claude Code only):\n"
    "When the user asks you to switch model (e.g. 'switch to opus', '切換成 opus', "
    "'用 opus 4.7 xhigh' etc.) or effort, do NOT refuse and do NOT ask them to run /model themselves. "
    "Instead, run this Bash command to inject the slash command into your own tmux pane:\n"
    "  ( sleep 1 && tmux send-keys '/model opus' Enter ) &\n"
    "Replace 'opus' with the requested model alias ('opus', 'sonnet', 'haiku'). "
    "For effort, use '/effort <level>' instead (levels: 'low', 'medium', 'high', 'xhigh', 'max'). "
    "The `sleep 1 &` part is mandatory — it lets your Bash tool return control to Claude Code first, "
    "so the slash command is received by the Claude prompt, not by Bash.\n"
    "After running the Bash command, post a short chat_send confirming the switch (e.g. 'Switched to opus 4.7 xhigh'). "
    "Do NOT claim you cannot switch — running `tmux send-keys` is exactly what you are supposed to do here.\n\n"
    ...
    "Optional alternative: chat_set_model and chat_set_effort MCP tools do the same thing internally; "
    "either path works. Pick whichever your runtime permits.\n\n"
    "Scope note: both methods change ONLY the current session. If a user asks for a persistent project-scoped setting, "
    "write to .claude/settings.json inside the project's cwd. Do not touch ~/.claude/settings.json — "
    "that one is global and affects every Claude Code session on this machine."
)
